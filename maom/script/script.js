...
function generate_poll(poll) {
    ...
    for (var i=0; i<options.length; i++){
        ...
        $option.append(generateProgressBar(options[i]["id"], pollTotal, options[i]["totalAmountRaised"], false, options[i]["name"], true));
        $div.append($option)
    }
    return($div);
}
...
$(document).ready(function(){
    ...
    $.get('script/tiltify.py?op=incentives', null, function(e){
	    ...
	    if (![52110,52141].includes(milestones[i]["id"])) $("#milestones").append(generate_milestone(milestones[i]));
        }
